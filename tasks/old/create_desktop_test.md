# Task: Create desktop_test.sh — Desktop Recycling Test Script

## Goal
New standalone script for desktop PC testing. No interactive tests
(screen/audio/camera/keyboard/touchpad). Operator enters ID, script
auto-collects hardware info, uploads to CearTrack, done.

## Workflow

```
Boot Live USB → run desktop_test.sh
    ↓
Prompt: Operator ID (one-time text input)
    ↓
Auto-collect (no human interaction):
  - System info (vendor/model/serial)
  - CPU
  - Memory
  - Storage (SMART + SSD health grade — reuse laptop logic)
  - GPU (desktop-specific, via lspci)
  - Network (wired only)
  - PSU info if readable
    ↓
Build JSON → upload to CearTrack /desktop/api/upload
    ↓
Print summary → exit (no further interaction)
```

## File: `desktop_test.sh`
...
while IFS= read -r disk; do
  ...
  DISK_JSON+="{\"device\":\"$name\",\"model\":\"${model:-unknown}\",\"size\":\"${size:-unknown}\",\"type\":\"$disk_type\",\"smart\":\"$smart\",\"power_on_hours\":\"${power_hours:-unknown}\",\"ssd_health_percent\":\"${SSD_HEALTH_PCT}\",\"ssd_grade\":\"${SSD_GRADE}\",\"ssd_available_spare\":\"${SSD_AVAIL_SPARE}\",\"ssd_data_written\":\"${SSD_DATA_WRITTEN}\"}"
  first=0
...
while IFS= read -r line; do
  ...
  # Fallback: try glxinfo if available (requires X, often not present on Live USB)
...
if [[ "$OVERALL" == "PASS" ]]; then
  ...
  echo -e "  ${RED}${BOLD}RESULT: ✗ FAIL — Replace storage before resale${NC}"
fi
echo ""
...
## Server Side: New `desktop` Module

Same pattern as laptop module:
1. `modules/desktop/module.py` — `DesktopModule` class extending `TestModule`
2. `modules/desktop/schema.json` — display schema (System/CPU/Memory/Storage/GPU/Network/PSU sections)
3. Auto-registered by `core/module_registry.py` — no other code changes needed
4. Same `@module_required('desktop')` protection pattern

Reuse `core/storage.py` write_envelope() unchanged — works for any module.

## Constraints
- No interactive prompts except Operator ID
- GPU section is INFO COLLECTION ONLY — no stress test, no benchmark
  (GPU stress testing is a separate independent module, not part of this script)
- Reuse exact SSD grading logic from laptop_test.sh (same thresholds A/B/C/D)
- Disk SMART FAILED → overall_result = FAIL, summary mentions "replace storage"
- Run `bash -n desktop_test.sh` after creation
