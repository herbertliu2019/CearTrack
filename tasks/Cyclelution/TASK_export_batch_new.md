...
- 顶部：标题 + 条数
- 日期筛选下拉（按 wipe 日期），仅缩小视野，不作为选择依据
- 记录列表，每行：Grade 色块 / SN / 容量 / 右箭头。整行可点击加入
- **已加入的记录从左栏消失**，两栏不重复
...
- **倒序**列表（最新加入在最上），每行：slot_no / Grade 色块 / SN / 移除按钮
- 空态：`Pick from the left, or scan a drive`

**底部操作条：**

- 左侧计数 `12 drives selected`，无分母
- 右侧：清空队列（图标）、`Slot sheet`、`Export xlsx`
