...
class GpuModule(TestModule):
    ...
    def validate(self, raw_payload: dict):
        ...
        result = str(raw_payload.get("overall_result", "")).upper()
        if result not in VALID_RESULTS:
            return False, f"Invalid overall_result: {result!r} (expected one of {sorted(VALID_RESULTS)})"
        return True, "OK"
...
def api_today():
    ...
    info_only = sum(1 for r in rows if r["overall_result"] == "INFO_ONLY")
    # pass_rate is computed over FULL-tier records only (PASS+WARN+FAIL)
    full_tier = passed + warned + failed
    pass_rate = round(passed / full_tier * 100, 1) if full_tier else 0.0
...
def api_stats():
    ...
    info_only = sum(1 for r in records if r.get("overall_result") == "INFO_ONLY")
    # pass_rate computed over FULL-tier records only (PASS+WARN+FAIL)
    full_tier = passed + warned + failed

    by_vendor: dict[str, int] = {}
    by_station_map: dict[str, int] = {}
    ...
    # Per-station per-day breakdown for station-grouped UI
    station_daily: dict[str, dict] = defaultdict(lambda: {
        "name": "", "total": 0, "passed": 0, "warned": 0, "failed": 0, "info_only": 0,
        "days": defaultdict(lambda: {"total": 0, "passed": 0, "warned": 0, "failed": 0, "info_only": 0}),
    })
    ...
    for r in records:
        ...
        # ── Station aggregation ──────────────────────────────
        st_name = (p.get("test_info", {}) or {}).get("test_station") or "Unknown"
        by_station_map[st_name] = by_station_map.get(st_name, 0) + 1
        # per-station per-day aggregation for grouped Daily Breakdown
        sd = station_daily[st_name]
        sd["name"] = st_name
        sd["total"] += 1
        sd_day = sd["days"][(r.get("timestamp") or "")[:10]]
        sd_day["total"] += 1
        _r_sd = r.get("overall_result", "")
        if _r_sd == "PASS":      sd["passed"]    += 1; sd_day["passed"]    += 1
        elif _r_sd == "WARN":    sd["warned"]    += 1; sd_day["warned"]    += 1
        elif _r_sd == "FAIL":    sd["failed"]    += 1; sd_day["failed"]    += 1
        elif _r_sd == "INFO_ONLY": sd["info_only"] += 1; sd_day["info_only"] += 1
    ...
    # Build by_station_daily output: per-station list with nested days
    by_station_daily = []
    for st in station_daily.values():
        days_list = []
        for d, dv in sorted(st["days"].items()):
            full_d = dv["passed"] + dv["warned"] + dv["failed"]
            days_list.append({
                "date":      d,
                "total":     dv["total"],
                "passed":    dv["passed"],
                "warned":    dv["warned"],
                "failed":    dv["failed"],
                "info_only": dv["info_only"],
                "pass_rate": round(dv["passed"] / full_d * 100, 1) if full_d else 0.0,
            })
        full_st = st["passed"] + st["warned"] + st["failed"]
        by_station_daily.append({
            "name":      st["name"],
            "total":     st["total"],
            "passed":    st["passed"],
            "warned":    st["warned"],
            "failed":    st["failed"],
            "info_only": st["info_only"],
            "pass_rate": round(st["passed"] / full_st * 100, 1) if full_st else 0.0,
            "days":      days_list,
        })
    by_station_daily.sort(key=lambda x: x["total"], reverse=True)
...
def api_filter():
    ...
    if station:
        records = [r for r in records
                   if ((r.get("payload",{}).get("test_info",{}) or {}).get("test_station","") or "Unknown") == station]
    if reason:
    ...
    return jsonify({
        ...
        "filter": {
            "period": period, "vendor": vendor, "subvendor": subvendor,
            "station": station, "fail_reason": reason,
